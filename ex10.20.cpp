...
void string_to_vec(vector<string> &str_vec, string story, vector<string>::size_type sz){
	...
	auto word_ct = count_if(str_vec.begin(), str_vec.end(), [sz](const string &word) {return word.size() >= sz;}); 
	cout << word_ct << endl;
	auto word_req = stable_partition(str_vec.begin(), str_vec.end(), [sz](const string &s1){return s1.size() >= sz;});
