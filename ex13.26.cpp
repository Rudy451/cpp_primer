...
public:
	...
	StrBlob& operator=(const StrBlob& orig){data = orig.data;}
...
int main(){
	...
	StrBlob First({"Happy", "Happy", "Joy", "Joy"});	
	cout << First.index(3) << endl;
	StrBlob Second(First);
	cout << Second.index(3) << endl;
	StrBlob Third(Second);
	Third.action("sub");
	Third.action("add", "Man");
	First = Third;
	cout << First.index(3) << endl;
	cout << Second.index(3) << endl;
