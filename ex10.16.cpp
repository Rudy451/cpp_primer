...
using std::cout; using std::endl; using std::vector; using std::string; using std::find_if;
...
void string_to_vec(vector<string> &str_vec, string story, vector<string>::size_type sz){
	...
	auto word_req = find_if(str_vec.begin(), str_vec.end(), [sz](const string &s1){return s1.size() >= sz;});
	for_each(word_req, str_vec.end(), [](const string &s){cout << s << " ";});
	cout << endl;
}
