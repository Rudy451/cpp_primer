...
struct TextQuery{
	...
	// Query function
	QueryResult query(string s, QueryResult spot, int start = 0, int end = 1000000);
...
QueryResult TextQuery::query(string s, QueryResult spot, int start, int end){
...
private:
	...
	virtual QueryResult eval(TextQuery& t, int start = 0, int end = 1000000) = 0;
...
public:
	...
	QueryResult eval(TextQuery& t, int start = 0, int end = 1000000) const{return q->eval(t, start, end);}
private:
...
class WordQuery: public Query_base {
	...
	QueryResult eval(TextQuery& t, int start = 0, int end = 1000000){return t.query(query_word, ex, start, end);}
...
class NotQuery: public Query_base {
	...
	QueryResult eval(TextQuery& text, int start = 0, int end = 1000000);
	Query query;

};

QueryResult NotQuery::eval(TextQuery& text, int start, int end){
	
	auto result = query.eval(text, start, end);
	QueryResult Temp;
...
QueryResult AndQuery::eval(TextQuery& text, int start, int end){

	auto left = lhs.eval(text, start, end);
	auto right = rhs.eval(text, start, end);
	set<int> ret_lines;
...
QueryResult OrQuery::eval(TextQuery& text, int start, int end){
...
void print_and_save(Query &q, TextQuery &tq, vector<Query> &q_vec, int start = 0, int end = 1000000){

	print(cout, q.eval(tq, start, end));
	q_vec.push_back(q);	
...
//	runQueries(i_file);
	...
	print_and_save(q, tq, q_vec, 2, tq.page.size());
