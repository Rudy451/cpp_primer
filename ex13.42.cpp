...
struct TextQuery{
	...
	// Constructor
	TextQuery(StrVec input, ifstream &ip): i_file(ip){
		for(auto i = input.get_first(); i != input.get_last(); ++i){
			page.push_back(*i);
		}
	}
...
void runQueries(StrVec input, ifstream &infile){

	TextQuery tq(input, infile);
	QueryResult qry;
...
int main(int argc, char *argv[]){
	...
	StrVec stuff({"First", "Second", "Third", "Fourth"}); 
	ifstream i_file(argv[1]);
	runQueries(stuff, i_file);
