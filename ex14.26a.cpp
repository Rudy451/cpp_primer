...
public:
	...
	const string &operator[](int num) const;

	//const string operator[](size_t num) const {return *(beg() + num);}

	size_tp size() const {return data->size();}
	bool data_compare(const StrBlob &Other) const {return *(this->data) == *(Other.data);}
...
string &StrBlob::operator[](int num){
	auto temp = data->begin(); 
	advance(temp, num); 
	return *(temp);
}
...
const string &StrBlob::operator[](int num) const {
	auto temp = data->begin(); 
	advance(temp, num); 
	return *(temp);
}
...
bool operator==(const StrBlob &lhs, const StrBlob &rhs){
	...
	return((lhs.size() == rhs.size()) && lhs.data_compare(rhs));
...
int main(){
	...
	StrBlob first({"My", "Name"});
	StrBlob second({"My", "Name"});
	cout << first[1] << endl;
